...
export default class VLineMailServer extends EventEmitter {
  ...
  onMessage(data) {
    ...
    if (messageType === 'NON_SPECIFIC') {
      this.emit('non_specific', message)

      return {
        messageType: messageType.toLowerCase(), text: message
      }
    }
    ...
    this.emit('message', messageType.toLowerCase(), serviceData, specificData)
